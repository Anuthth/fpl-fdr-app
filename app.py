...
# --- RESTORED: Easy Run Finder Function ---
...
if ratings_df is not None and fixtures_df is not None:
    ...
    # Common AgGrid setup
    gb = GridOptionsBuilder.from_dataframe(master_df.reset_index().rename(columns={'index': 'Team'}))
    ...
    with tab1:
        ...
        gb.configure_dataframe(df_display.reset_index().rename(columns={'index': 'Team'})) # Reconfigure with sorted data
        gb.configure_column("Total Difficulty", width=120); gb.configure_column("Total xG", hide=True); gb.configure_column("Total CS", hide=True)
        ...
        for gw in range(start_gw, end_gw + 1):
            gw_col = f"GW{gw}"; gb.configure_column(gw_col, headerName=gw_col, valueGetter=f"data['{gw_col}'] ? data['{gw_col}'].display : ''", cellStyle=jscode_fdr, width=100)
        
        AgGrid(df_display, gridOptions=gb.build(), allow_unsafe_jscode=True, theme='streamlit-dark', height=(len(df_display) + 1) * 35, fit_columns_on_grid_load=True, key='fdr_grid')

    with tab2:
        st.subheader("Projected Goals (Higher is better for attackers)")
        df_display = master_df.sort_values(by='Total xG', ascending=False)
        gb.configure_dataframe(df_display.reset_index().rename(columns={'index': 'Team'}))
        gb.configure_column("Total xG", width=120, valueFormatter="data['Total xG'].toFixed(2)"); gb.configure_column("Total Difficulty", hide=True); gb.configure_column("Total CS", hide=True)
        ...
        for gw in range(start_gw, end_gw + 1):
            gw_col = f"GW{gw}"; gb.configure_column(gw_col, headerName=gw_col, valueGetter=f"data['{gw_col}'] ? data['{gw_col}'].xG.toFixed(2) : ''", cellStyle=jscode_xg, width=100)
        
        AgGrid(df_display, gridOptions=gb.build(), allow_unsafe_jscode=True, theme='streamlit-dark', height=(len(df_display) + 1) * 35, fit_columns_on_grid_load=True, key='xg_grid')
        
    with tab3:
        st.subheader("Projected Clean Sheets (Higher is better for defenders)")
        df_display = master_df.sort_values(by='Total CS', ascending=False)
        gb.configure_dataframe(df_display.reset_index().rename(columns={'index': 'Team'}))
        gb.configure_column("Total CS", width=120, valueFormatter="(data['Total CS'] * 100).toFixed(1) + '%'"); gb.configure_column("Total Difficulty", hide=True); gb.configure_column("Total xG", hide=True)
        ...
        for gw in range(start_gw, end_gw + 1):
            gw_col = f"GW{gw}"; gb.configure_column(gw_col, headerName=gw_col, valueGetter=f"data['{gw_col}'] ? (data['{gw_col}'].CS * 100).toFixed(0) + '%' : ''", cellStyle=jscode_cs, width=100)
        
        AgGrid(df_display, gridOptions=gb.build(), allow_unsafe_jscode=True, theme='streamlit-dark', height=(len(df_display) + 1) * 35, fit_columns_on_grid_load=True, key='cs_grid')

    # --- RESTORED: Easy Run Finder Section ---
    st.sidebar.header("Easy Run Finder")
    st.sidebar.info("Find upcoming periods of 3+ easy/neutral fixtures (FDR 1-3).")
    teams_to_check = st.sidebar.multiselect("Select teams to find runs for:", options=PREMIER_LEAGUE_TEAMS, default=[])
