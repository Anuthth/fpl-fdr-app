...
if ratings_df is not None and fixtures_df is not None:
    ...
    with tab2:
        ...
        jscode = JsCode("""function(params) { const cellData = params.data[params.colDef.field]; if (cellData && cellData.xG !== undefined) { const xG = cellData.xG; let bgColor; if (xG >= 2.0) { bgColor = '#63be7b'; } else if (xG >= 1.75) { bgColor = '#95d2a6'; } else if (xG >= 1.2) { bgColor = '#bfe4cb'; } else if (xG >= 1.0) { bgColor = '#d3ecdb'; } else { bgColor = '#ffffff'; } const textColor = (xG >= 0.8 && xG < 1.2) ? '#31333F' : '#FFFFFF'; return {'backgroundColor': bgColor, 'color': textColor, 'fontWeight': 'bold'}; } return {'textAlign': 'center', 'backgroundColor': '#444444'}; };""")
