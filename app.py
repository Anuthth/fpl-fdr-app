...
if ratings_df is not None and fixtures_df is not None:
    ...
    with tab3:
        ...
        gb = GridOptionsBuilder.from_dataframe(df_display)
        gb.configure_column("Team", pinned='left', cellStyle={'textAlign': 'left'}, flex=2, sortable=True)
        gb.configure_column("xCS", header_name="Expected CS (xCS)", valueFormatter="data['xCS'].toFixed(0)", flex=1.5, type=["numericColumn"])
        ...
        for col in gw_columns:
            gb.configure_column(col, headerName=col, valueGetter=f"data['{col}'] ? (data['{col}'].CS * 100).toFixed(2) + '%' : ''", comparator=JsCode(comparator_template.format(gw_col=col)), cellStyle=jscode, flex=1)
