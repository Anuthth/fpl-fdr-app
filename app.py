...
if ratings_df is not None and fixtures_df is not None:
    ...
    with tab1:
        ...
        for gw in range(start_gw, end_gw + 1):
            gw_col = f"GW{gw}"; gb.configure_column(gw_col, headerName=gw_col, valueGetter=f"data['{gw_col}'] ? data['{gw_col}'].display : ''", comparator=JsCode(comparator_template.format(gw_col=gw_col)), cellStyle=jscode_fdr, flex=1)
    ...
    with tab2:
        ...
        gb = GridOptionsBuilder.from_dataframe(df_display)
        gb.configure_column("Team", pinned='left', cellStyle={'textAlign': 'left'}, flex=2, sortable=True)
        gb.configure_column("Total xG", valueFormatter="data['Total xG'].toFixed(2)", flex=1.5, type=["numericColumn", "numberColumnFilter", "customNumericFormat"], precision=2)
        ...
        for gw in range(start_gw, end_gw + 1):
            gw_col = f"GW{gw}"; gb.configure_column(gw_col, headerName=gw_col, valueGetter=f"data['{gw_col}'] ? data['{gw_col}'].xG.toFixed(2) : ''", comparator=JsCode(comparator_template.format(gw_col=gw_col)), cellStyle=jscode, flex=1)
    ...
    with tab3:
        ...
        gb = GridOptionsBuilder.from_dataframe(df_display)
        gb.configure_column("Team", pinned='left', cellStyle={'textAlign': 'left'}, flex=2, sortable=True)
        gb.configure_column("xCS", header_name="Expected CS (xCS)", valueFormatter="data['xCS'].toFixed(2)", flex=1.5, type=["numericColumn", "numberColumnFilter", "customNumericFormat"], precision=2)
        ...
        for gw in range(start_gw, end_gw + 1):
            gw_col = f"GW{gw}"; gb.configure_column(gw_col, headerName=gw_col, valueGetter=f"data['{gw_col}'] ? (data['{gw_col}'].CS * 100).toFixed(0) + '%' : ''", comparator=JsCode(comparator_template.format(gw_col=gw_col)), cellStyle=jscode, flex=1)
