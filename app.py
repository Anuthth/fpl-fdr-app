...
if ratings_df is not None and fixtures_df is not None:
    ...
    with tab1:
        ...
        gb.configure_column("Team", pinned='left', cellStyle={'textAlign': 'left'}, flex=2)
        ...
        for col in gw_columns:
            gb.configure_column(col, headerName=col, valueGetter=f"data['{col}'] ? data['{col}'].display : ''", comparator=JsCode(comparator_template.format(gw_col=col)), cellStyle=jscode_fdr, flex=1)
    ...
    with tab2:
        ...
        for col in gw_columns:
            gb.configure_column(col, headerName=col, valueGetter=f"data['{col}'] ? data['{col}'].xG.toFixed(2) : ''", comparator=JsCode(comparator_template.format(gw_col=col)), cellStyle=jscode_xg, flex=1)
    ...
    with tab3:
        ...
        for col in gw_columns:
            gb.configure_column(col, headerName=col, valueGetter=f"data['{col}'] ? (data['{col}'].CS * 100).toFixed(0) + '%' : ''", comparator=JsCode(comparator_template.format(gw_col=col)), cellStyle=jscode_cs, flex=1)
